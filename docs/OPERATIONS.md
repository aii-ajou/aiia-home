...
>
> 내용을 입력한 뒤 **Save**를 누르면 저장됩니다. 새 소식·연구자·연구센터·메뉴는 기본 상태가 **Draft(임시저장)**입니다. **Draft는 저장 실패가 아니라, 저장은 되었지만 홈페이지에는 아직 공개하지 않은 상태**입니다.
>
...
사진은 연구자 4:5 세로형, 소식은 가로형을 권장합니다. **새 연구자·소식·연구센터·메뉴는 이름/제목 등 내용만 입력하면 됩니다.** 파일 이름은 내부 고유번호로 자동 생성하며 별도 입력하지 않습니다. 한글 이름이나 동명이인도 사용할 수 있고, 저장 후 이름을 수정해도 기존 주소는 유지됩니다. 현재 날짜를 바꿔 미래 날짜로 입력해도 예약 게시되지는 않습니다.
