...
>
> 내용을 입력한 뒤 **Save**를 누르면 저장됩니다. 새 소식·연구자·연구 분야·메뉴는 기본 상태가 **Draft(임시저장)**입니다. **Draft는 저장 실패가 아니라, 저장은 되었지만 홈페이지에는 아직 공개하지 않은 상태**입니다.
>
> **바로 공개하려면:** 해당 항목을 열고 아래쪽 **게시 상태 → 게시(Published)**로 변경한 뒤 **Save**를 누르세요. 자동 배포가 완료되면 홈페이지에 표시됩니다. 이미 게시된 내용이나 페이지 문구·사진을 수정한 경우에도 저장 후 배포가 완료되면 반영됩니다.
>
> 이메일은 `name@ajou.ac.kr`처럼 주소만 입력하세요. 홈페이지에서는 메일 작성 링크가 자동으로 붙습니다.
>
...
3. 새 항목은 기본적으로 **Draft(임시저장)**입니다. **Save**하면 저장되지만 아직 공개되지는 않습니다. 즉시 공개하려면 항목 아래쪽 **게시 상태 → 게시(Published)**를 선택하고 **Save**합니다. 숨기려면 **보관(Archived)**으로 저장합니다.
4. 저장 → GitHub 변경 → 자동 배포 완료 후 공개됩니다. 연속 저장은 최신 내용을 묶어 배포할 수 있습니다. 게시된 글 수정도 저장 즉시 배포 대상입니다.

사진은 연구자 4:5 세로형, 소식은 가로형을 권장합니다. **새 연구자·소식·연구 분야·메뉴는 이름/제목 등 내용만 입력하면 됩니다.** 파일 이름은 내부 고유번호로 자동 생성하며 별도 입력하지 않습니다. 한글 이름이나 동명이인도 사용할 수 있고, 저장 후 이름을 수정해도 기존 주소는 유지됩니다. 현재 날짜를 바꿔 미래 날짜로 입력해도 예약 게시되지는 않습니다.

신규 작성 화면에 파일 이름 입력창이 보이거나 `Invalid extension` 오류가 나면, 작성 중인 내용을 먼저 복사해 두고 CMS를 새로고침한 뒤 새 항목 작성을 다시 엽니다. 업데이트 전 설정이 열린 화면에 남아 있을 수 있습니다.
...
**이메일 링크:** 홈페이지의 이메일 주소나 이메일 문의 버튼은 `mailto:` 링크로 기본 메일 작성기를 엽니다. 클릭해도 반응이 없다면 해당 기기·브라우저에 기본 메일 앱 또는 웹메일 연결이 설정되어 있는지 확인합니다. 메일이 자동 발송되는 것은 아닙니다.

## 사진·첨부와 미사용 보관함

미디어 메뉴는 **연구자 사진 / 소식 사진 / 공통 사진 / 첨부 문서 / 미사용 사진 보관함 / 미사용 첨부 보관함**으로 나뉩니다. 연구자 정보에서 올리면 연구자 사진에, 소식의 대표 사진은 소식 사진에, 첫 화면·공유 미리보기·지도는 공통 사진에 저장됩니다. 이미 업로드한 파일은 **Select(선택)**로 재사용하세요.

사진 필드의 휴지통은 해당 항목과 파일의 **연결 해제**입니다. 실제 파일은 즉시 삭제되지 않습니다. 콘텐츠 저장·파일 업로드 후와 6시간마다 전체 콘텐츠의 사용 여부를 점검하며, **처음 미사용으로 확인된 뒤 24시간이 지난 파일**을 다음 점검 때 보관함으로 옮깁니다. 새 사진을 업로드한 뒤에는 콘텐츠도 Save하세요. 24시간 이상 저장하지 않은 편집 화면은 새로고침하고 파일을 다시 확인하세요.

- 임시저장(Draft)·보관(Archived) 상태의 콘텐츠, 본문 링크, 공통 설정 중 한 곳에서라도 연결되어 있으면 사용 중입니다.
- 보관함 파일은 자동 삭제하지 않습니다. **미디어 → 미사용 사진 보관함 / 미사용 첨부 보관함**에서 운영자가 확인 후 필요 없는 파일을 삭제합니다. 이 삭제는 저장소의 현재 파일을 제거하므로 복구가 필요하면 개발자에게 Git 이력 복원을 요청하세요.
- 다시 쓰려면 보관함의 파일을 내려받고, 사용할 콘텐츠의 사진·첨부 필드에서 업로드한 뒤 Save합니다. 새 파일이 정상 표시되면 보관함의 이전 사본은 운영자가 정리할 수 있습니다.
- 보관함에는 옛 폴더 구조가 남을 수 있습니다. 현재 사용하는 파일의 저장 위치는 위 네 종류로 통일됩니다. 중복 파일도 사용 중인 경로를 유지하며, 연결이 없는 사본만 보관합니다.
- 보관함은 비공개 콘텐츠 저장소에만 보관하며 홈페이지 배포에 포함하지 않습니다. Git 이력과 이미 내려받은 사본까지 삭제하는 기능은 아닙니다.

개발자: `cms/pages.yml` → `.pages.yml`, `cms/publish-site.yml` → `.github/workflows/publish-site.yml`, `cms/reconcile-media.mjs` → `scripts/reconcile-media.mjs`를 콘텐츠 저장소에 함께 설치합니다. 사용 파일은 `uploads/members`, `uploads/news`, `uploads/images`, `uploads/documents`, 미사용 파일은 `media-archive/images`, `media-archive/documents`에 저장됩니다. `media-state.json`은 미사용 확인 시각과 파일 해시를 기록합니다. 같은 경로의 파일 내용이 바뀌면 유예 시간을 다시 계산합니다.

수동 점검은 `node cms/reconcile-media.mjs .content-repository`(변경 없음), 적용은 뒤에 `--apply`를 붙입니다. `--archive-now`는 유예 없이 기존 미사용 파일을 보관하는 최초 이전용 옵션입니다. 파일 이동과 참조 경로 변경은 하나의 커밋으로 반영합니다. 정리 중 다른 편집이 저장되어 push가 거부되면 오래된 결과를 rebase하지 말고, 최신 콘텐츠를 받아 점검부터 다시 실행합니다. 자동 정리에는 콘텐츠 저장소의 `GITHUB_TOKEN` Contents 쓰기 권한을 사용하며 별도 토큰은 필요 없습니다. 자동 정리 커밋 후 배포 요청도 같은 workflow에서 실행합니다.

## 조직도 검토와 승인

**연구 분야**는 분야별 소개, **조직도**는 실제 운영 조직의 편집 메뉴입니다. 조직도는 내용을 수정해 **Save → 그림 자동 생성 완료 → CMS 새로고침 → 조직도 검토 그림 확인 → 공개 승인 선택 → Save** 순서로 공개합니다. 승인·배포 전에는 기존 조직도를 유지합니다.

이름·종류·책임자·표시 여부·배치 순서·조직도 링크 등 그림에 영향을 주는 변경만 그림을 생성합니다. 역할 설명·사진만 바꾸면 기존 그림을 재사용하고 자동 반영합니다. 다만 조직도 승인 대기 중이면 해당 조직의 설명·사진도 함께 보류합니다. 공개 중인 사진은 승인본에서도 사용 여부를 확인하므로 미사용 보관함으로 옮기지 않습니다.

CMS **조직도 검토 그림**은 읽기 전용입니다. 자동 생성 뒤 화면에 예전 그림이나 승인 선택지만 보이면 CMS를 새로고침하세요. 그림 생성 작업이 실패하면 기존 공개본이 유지됩니다. 상세 설계와 설치는 [조직도 편집·검토·공개](ORGANIZATION-PREVIEW.md)를 참고하세요.
